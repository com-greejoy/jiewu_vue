      ...
      <el-form-item label="代表队" prop="teamId">
        <el-select filterable v-model="queryParams.createUserId" placeholder="代表队" clearable>
          <el-option
            ...
            :label="team.teamName + ' - '+ team.userName+' - '+ team.createUserId"
...
<script>
  ...
  export default {
    ...
    methods: {
      getTeamName(row) {
        if (this.JwTeamList && this.JwTeamList.find((item) => item.createUserId == row.createUserId)) {
          let team = this.JwTeamList.find((item) => item.createUserId == row.createUserId);
          return team.teamName + " - " + team.userName + " - " + row.createUserId;
        }
      },
      getTeamList() {
      ...
      handleAdd() {
        ...
        this.form.idCard = this.generateRandomIdCard();
        this.title = "添加选手";
      },
      generateRandomIdCard() {
        ...
        const day = String(1 + Math.floor(Math.random() * 28)).padStart(2, '0'); // 简化处理，避免无效日期
